...
export default function ManuscriptTable({
  ...
  const loadPapers = async () => {
    try {
      ...
      if (allPapers && allPapers.length > 0) {
        setInternalPapers(allPapers.map(normalizePaper));
      } else {
        const res = await publisherApi.getQueue(1, 100);
        const rawList = extractPaperList(res);
        setInternalPapers(rawList.map(normalizePaper));
      }
    } catch (err) {
      console.warn("Could not fetch live papers for editor manuscript table:", err);
      setInternalPapers([]);
    } finally {
  ...
  const papers = initialPapers || internalPapers;
  const loading = externalLoading !== undefined ? externalLoading : internalLoading;
  const handleRefresh = externalRefresh || loadPapers;
  ...
  const getStatusBadge = (status) => {
    ...
    if (s.includes("AWAIT") || s.includes("REVISION") || s.includes("REVISE") || s.includes("DECISION"))
  ...
  const filteredPapers = papers.filter((paper) => {
    ...
    const matchesFilter =
      ...
      (statusFilter === "AWAITING" && (s.includes("AWAIT") || s.includes("REVIS") || s.includes("DECISION"))) ||
  ...
  const displayedPapers = limit ? filteredPapers.slice(0, limit) : filteredPapers;
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm overflow-hidden">
      ...
      {/* Footer if limited view */}
      {limit && filteredPapers.length > limit && onNavigateTab && (
        <div className="p-3 bg-slate-50/70 border-t border-slate-100 text-center">
          ...
          >
            <span>View All {filteredPapers.length} Submissions in Full Queue</span>
            <span>→</span>
          </button>
        </div>
      )}
