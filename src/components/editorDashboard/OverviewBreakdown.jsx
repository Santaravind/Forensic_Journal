...
export default function OverviewBreakdown({
  ...
  const awaitingDecision = papers.filter((p) => {
    const s = (p.status || "").toUpperCase();
    return s.includes("AWAIT") || s.includes("REVIS") || s.includes("DECISION");
  }).length;
  ...
  const statusList = [
    {
      ...
      icon: FileText,
      hint: "Pending initial editorial check",
    },
    {
      ...
      icon: Clock,
      hint: "Assigned to peer reviewers",
    },
    {
      label: "Awaiting Decision / Revision",
      ...
      icon: AlertCircle,
      hint: "Reviews in • Judgment pending",
    },
    {
    ...
    },
    {
      ...
      icon: XCircle,
      hint: "Declined after evaluation",
    },
  ];
        ...
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-base font-bold text-slate-900">
              Manuscript Editorial Pipeline & Distribution
            </h3>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Real-time status breakdown of all {total} submissions across the editorial review lifecycle.
          </p>
        </div>
