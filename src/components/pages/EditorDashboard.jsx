...
import Header from "../editorDashboard/Header";
import StatsSection from "../editorDashboard/StatsSection";
import QuickActions from "../editorDashboard/QuickActions";
import OverviewBreakdown from "../editorDashboard/OverviewBreakdown";
import ManuscriptTable from "../editorDashboard/ManuscriptTable";
import PeerReviewView from "../editorDashboard/PeerReviewView";
import EditorialGuidelines from "../editorDashboard/EditorialGuidelines";
...
export default function EditorDashboard() {
  ...
  const loadAllPapers = async () => {
    try {
      ...
      console.warn("Could not fetch papers for editor dashboard:", err);
      setPapers([]);
    } finally {
      setLoading(false);
    }
  ...
  const reviewCount = papers.filter((p) => {
    const s = (p.status || "").toUpperCase();
    return s.includes("REVIEW") || s.includes("AWAIT") || s.includes("DECISION") || s.includes("REVIS");
  }).length;
        ...
        <div className="p-4 sm:p-6 space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/90 p-2 shadow-xs flex items-center justify-between gap-2 overflow-x-auto">
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              >
                <Clock size={14} />
                <span>Peer Review & Decisions</span>
                {reviewCount > 0 && (
                  <span
                    className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded-full ${
                      activeTab === "Decisions" ? "bg-white/20 text-white" : "bg-amber-100 text-amber-700"
                    }`}
                  >
                    {reviewCount}
                  </span>
          ...
          {/* TAB 4: DEDICATED BLOG & EDITORIAL MODERATION */}
          {activeTab === "Blog" && (
            <div className="animate-in fade-in duration-200">
              <BlogManagement />
            </div>
          ...
          {/* TAB 6: ANNOUNCEMENTS & BROADCASTS */}
          {activeTab === "Announcements" && (
            <div className="animate-in fade-in duration-200">
          ...
          {/* TAB 7: EDITORIAL GUIDELINES & ETHICS */}
          {activeTab === "Guidelines" && (
            <div className="animate-in fade-in duration-200">
              <EditorialGuidelines />
            </div>
